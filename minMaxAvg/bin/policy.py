...
class Policy(object):
    def __init__(self, env, fileini, seed, test):
        ...
        self.max_fitness_weight = 0 #the weight of the best finess trial at the final fitness computation
        self.avg_fitness_weight = 1 #the weight of the average finess on all trials at the final fitness computation
        self.min_fitness_weight = 0 #the weight of the worst finess trial at the final fitness computation
...
class BulletPolicy(Policy):
    ...
    def rollout(self, ntrials, render=False, seed=None, step=None, maxmsteps=None):   # evaluate the policy for one or more episodes 
        ...
        if (self.test > 0 and ntrials > 1):
           print("Max fit %.2f Average Fit %.2f Min Fit %.2f Steps %.2f " % (vect_rews.max(),vect_rews.mean(),vect_rews.min(), steps/float(ntrials)))
           print(f"Computed Weighted Fitness: {rews} - Weights: max: {self.max_fitness_weight} avg: {self.avg_fitness_weight} min:{self.min_fitness_weight}")        
           print(f"TEST MODE - returning the average fitness")
           rews = vect_rews.mean()
        return rews, steps
